...
rule local2@local($x) :- local@local($x);
  ...
  def test_delay_fact_loading
    ...
    assert_equal ["rule join_delegated@p0($x) :- local@test_delay_fact_loading($x), delegated@p1($x), delegated@p2($x), delegated@p3($x);",
      "rule local2@test_delay_fact_loading($x) :- local@test_delay_fact_loading($x);",
      "rule deleg_from_test_delay_fact_loading_1_1@p1($x) :- local@test_delay_fact_loading($x);",
    ...
    assert_equal [{:atom1=>"1"}, {:atom1=>"2"}, {:atom1=>"3"}, {:atom1=>"4"}],
      wl_obj.tables[:local_at_test_delay_fact_loading].map { |t| Hash[t.each_pair.to_a] },
