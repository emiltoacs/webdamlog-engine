...
module WLBud
  ...
  PATH_WLBUD = File.expand_path(File.dirname(__FILE__))
  PATH_LIB = File.expand_path("wlbud/", PATH_WLBUD)
  PATH_PROJECT = File.expand_path("..", PATH_WLBUD)
  PATH_CONFIG = File.expand_path("config/", PATH_PROJECT)
  RULE_DIR_NAME = "wlrule_to_bud"
  # No modification of the load path in a gem
  #$:.unshift PATH_WLBUD unless $LOAD_PATH.include?(PATH_WLBUD)
  #$:.unshift PATH_LIB unless $LOAD_PATH.include?(PATH_LIB)
  #$:.unshift PATH_CONFIG unless $LOAD_PATH.include?(PATH_CONFIG)
  ...
  #file project
  require "#{PATH_LIB}/wlprogram"
  require "#{PATH_LIB}/wlpacket"
  require "#{PATH_LIB}/wlerror"
  require "#{PATH_LIB}/wlchannel"
  require "#{PATH_LIB}/wlvocabulary"
  # file project: automatically generated by polyglot but could also be compiled
  # via tt
  require "#{PATH_LIB}/wlgrammar"
  #file tool project
  require "#{PATH_LIB}/tools/wltools"

  # :title:WLBud
  # WLBud is a Ruby Module that simulates WebdamLog
  # behavior using Bud.
  # :main:WLBud
  

  # TODO: remove class to force user to create a new class that include WLBud.
  # It is very unlikely that any user wants to instanciate this class directly
  ...
  class WL
    ...
    # @return the content returned by read_packet_channel at the begining og the tick (an array of WLPacketData)
    ...
    # * +pgfilename+ the filename containing the program (or any IO object readable)
    # * +options+ 
    #
    # ==== Options
    #
    ...
    def initialize (peername, pgfilename, options={})
      ...
      #Special bud parameter initialization
      if options[:mesure]
        @start_time = Time.now        
      end
      @peername=peername
      ...
      # can come into this Hash only from rule rewriting or come from a seed rule
      # that have new values at this tick.
      ...
      #        WLTools::Print_Tables.print_arg_tab(@wl_program.localrules, prefix+'localrules')
      #        WLTools::Print_Tables.print_arg_tab(@wl_program.rewrittenlocal, prefix+'rewrittenrules')
      #        WLTools::Print_Tables.print_arg_tab(@wl_program.delegations, prefix+'delegations')
      #        WLTools::Debug_messages.h2(WLTools::Debug_messages.end_comment debug_comment) if @options[:debug]
      #      end
      # All the relation of the program are stored in wlcollection and will be declared in make_program
      ...
      # - bud_meta.rb => rewrite_rule_block
      # Bloom programs consists of Bloom Blocks that are interpreted as 
      # instance methods by the bud compiler with a certain format (__bloom__.+ in regex).
      # What we do is create new such methods from our .wl file for the class
      # and give them an appropriate name, so that Bud believes these are methods
      # identical to his bloom do.
      ...
      #### WLBud:Begin remove from Bud
      # This list os now done in rewrite_strata method called just after
      #@declarations = self.class.instance_methods.select {|m| m =~ /^__bloom__.+$/}.map {|m| m.to_s}
      #### WLBud:End remove from Bud


      ### WLBud:Begin alternative to Bud
      #Rewrites local methods and adds references for modules and temp collections for class modules.
    ...
    def tick_internal
      ...
      # already in bud but I moved receive_inbound before all the stuff about app_tables, push_sorted_elements, ...
      receive_inbound
      ### WLBud:End adding to Bud
      ...
      begin
        ...
        if @options[:wl_test]
          # callback insertion of callback_step_received_on_chan
          # MArshalling is used to deep duplicate the object
...
#          pp "peer #{@peername} will process the content of a package #{packet_value.to_s}"
        ...
        #if @collection_added then call_state_methods ; @collection_added=false end # useless could be removed
        ...
        if @options[:debug] and @options[:trace]
          ...
          #logtab = @viz.class.send(:logtab)
          #dbm = logtab.class.send(:dbm)
    ...
    def builtin_state
      ...
      #scratch :readChannel, [:result]
      #this is the buffer that stores in all the facts to be sent to each peer.
      #scratch :buffer, [:dst,:facts]
    end
    ...
    def rewrite_strata
      ...
      @declarations = self.class.instance_methods.select {|m| m =~ /^__bloom__.+$/}.map {|m| m.to_s}
      #Erase all previous rules and dependence as we rebuild the graph from scratch
    ...
    #Assumes that the program runs in the background. Waits until
    #there is something in the chan to evaluate again. Facts are properly
    #received in their appropriate relation.
    #
    #    def run_loop
    #      count = 1
    #      while count < 100
    ...
    def translate_rule(wlrule)
      ...
      name = "webdamlog_#{@peername}_#{wlrule.index}"
    ...
    # Oldies doesn't work anymore with bud 0.9.1 we need to write rule in a file
    # to make it work.
    #
    # This method will translate one wlrule in parameter into
    # Bud string and assign it the rule number n. This bud string is then
    # evaluated using a ruby proc and a method is dynamically added to the bud
    # instance. It returns the generated name of the corresponding rule that
    # should be kept to refer to it in the future. TODO: line 4 of this function
    # change the wlrule.index for a unique rule identifier generated by wlbud
    #    def translate_rule(wlrule)
    #      str = "#{@wl_program.translate_rule_str(wlrule)}"
    #      the_block = eval("lambda {#{str}}")
    #      name = "_webdamlog_#{@peername}_#{wlrule.index}".to_sym
    #      if @options[:debug]
    #        puts "Method name is dynamically created into bud: #{name}\n \t#{str}\n"
    #        puts "here is the block \"lambda #{str}\" send to define a new method with bloom method\n"
    #        puts the_block.inspect
    #      end
    #      #      self.rule_init([name,proc])
    #      self.class.send(:bloom, name) {str}
    #    end
    ...
    def add_fact (fact_str)
      ...
      # REMOVE
      #fields=""
      #fact.contents.each {|f| fields<<"#{self.quotes(f)},"}
      #fields.slice!(-1)
      # eval ("self.#{fact.name}<=[[#{fields}]]")
    end
    ...
    #
    # This create the block called bootstrap containing the fact for the
    ...
    # The create_rule_blocks_rules method creates Bloom blocks from previously parsed
    # rules in @wlprogram, for each local rules ie.
...
#        tables.each { |t| pp "#{t}" }
    ...
    # for production.
    #
    # * :callback_step_received_on_chan called in the tick just after inbound has been flushed into chan
    # * :callback_step_write_on_chan, :callback_step_write_on_chan_2 two callback called just after writing on channel
    # * :callback_step_end_tick is called at the end of the tick with self as argument
    #
    # === return
  ...
  #This method formats a fact table to be
  #processed by WebdamExchange manager module.
  #
